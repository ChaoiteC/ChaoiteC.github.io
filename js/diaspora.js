    ...
    if ($('#preview').length) {
        ...
        cover.t.on('load', function() {
            ...
            if (swatches['DarkVibrant']) {
                $('#vibrant polygon').css('fill', '#006eb4'/*swatches['DarkVibrant'].getHex()*/)
                $('#vibrant div').css('background-color', '#006eb4'/*swatches['DarkVibrant'].getHex()*/)
            }
            if (swatches['Vibrant']) {
                $('.icon-menu').css('color', '#808080'/*swatches['Vibrant'].getHex()*/)
				$('.icon-search').css('color', '#808080'/*swatches['Vibrant'].getHex()*/)
            }
        })
        if (!cover.t.attr('src')) {
        ...
        switch (true) {
            ...
            case (tag.indexOf('switchmenu') != -1):
				...
				if(typed !== null)
                    ...
                    /*fetch('https://raw.githubusercontent.com/ChaoiteC/ChaoiteC.github.io/main/json/hitokoto.json')
                    ...
                    .then(data => {
                        ...
                        // 处理随机选择的数据
                        var str = randomData.hitokoto + " ——  By "		
                        var options = {
                        strings: [ 
                            str + randomData.from,
                        ],
